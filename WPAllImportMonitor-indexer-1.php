...
// Absolute path to WP-CLI – adjust if `which wp` shows a different path
    define( 'WPAIP_WPCLI', '/usr/local/bin/wp' );
    ...
    /**
     * Generic CLI launcher used by the JSF indexer.
     *
     * @param string $cmd        The full WP-CLI command to run.
     * @param string $prefix     Short slug used in the log-file name.
     * @return array{success:bool, exit_code:int, pid:string, log_file:string}
     */
    function wpaip_spawn_cli( $cmd, $prefix = 'cli_job' ) {

        // Re-use the same log directory we already create for imports
        $log_dir = plugin_dir_path( __FILE__ ) . 'import-logs/';
        if ( ! is_dir( $log_dir ) ) {
            mkdir( $log_dir, 0755, true );
        }

        $log_file = $log_dir . $prefix . '_' . gmdate( 'Ymd\THis' ) . '.log';

        // Wrap in setsid so it detaches; capture stdout+stderr; echo PID
        $full = sprintf(
        ...
        $output = [];
        $code   = 0;
        $pid    = exec( $full, $output, $code );

        error_log( "WPAIP: Spawned {$prefix}. pid={$pid}, exit_code={$code}, log={$log_file}" );
    ...
    function wpaip_after_xml_import($import_id, $import) {
        ...
        // Trigger JetSmartFilters re-index if this was import 15
        wpaip_maybe_run_jsf_indexer( $import_id );
    ...
    /**
     * Fire Jet Smart Filters full re-index once the last import (ID 15) finishes.
     *
     * @param int $import_id  The ID of the import that just completed.
     */
    function wpaip_maybe_run_jsf_indexer( $import_id ) {

        // 1) Only act when Import ID 15 (Congregations) completes
        if ( intval( $import_id ) !== 15 ) {
            return;
        }

        /**
         * 2) Build a WP-CLI eval that:
         *    a) loads the Jet Smart Filters plugin manually,
         *    b) triggers its normal bootstrap hooks,
         *    c) runs the new indexer.
         */
    /* ---------- build CLI code ---------- */
        $php_code = <<<'PHP'
        // Jet Smart Filters is already loaded by WP-CLI boot-strap.
        if ( class_exists( '\Jet_Smart_Filters\Indexes\Manager' ) ) {
            \Jet_Smart_Filters\Indexes\Manager::instance()->reindex_all();
            echo "JSF index rebuilt (Manager)\n";
            return;
        }

        // Fallback for VERY old JSF versions (< 2.3).
        if ( function_exists( 'jet_smart_filters' ) && method_exists( jet_smart_filters()->indexer, 'index_all' ) ) {
            jet_smart_filters()->indexer->index_all();
            echo "JSF index rebuilt (legacy)\n";
            return;
        }

        echo "JSF classes still not found – index NOT rebuilt\n";
        exit(1);
    PHP;
    ...
    $cmd = sprintf(
        '%s --path=%s eval %s',
        escapeshellarg( WPAIP_WPCLI ),
        escapeshellarg( ABSPATH ),
        escapeshellarg( $php_code )
    );
    
        // 3) Spawn the CLI process exactly like the import runs
        $launch = wpaip_spawn_cli( $cmd, 'jsf_index' );

        // 4) Log a breadcrumb so we can verify afterwards
        error_log(
            sprintf(
                'WPAIP: Launched Jet Smart Filters indexer. pid=%d, log=%s',
                $launch['pid'],
                $launch['log_file']
            )
        );
